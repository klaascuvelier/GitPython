...
class TestIndex(TestBase):
	...
	def test_index_mutation(self, rw_repo):
		...
		entries = index.reset(new_commit).add([BaseIndexEntry((10644, null_bin_sha, 0, new_file_relapath))], fprogress=self._fprogress_add)
		self._assert_entries(entries)
		self._assert_fprogress(entries)
		...
		fake_entry = BaseIndexEntry((120000, null_bin_sha, 0, fake_symlink_relapath))
		...
		full_index_entry = IndexEntry.from_base(BaseIndexEntry((120000, entries[0].binsha, 0, entries[0].path)))
		...
		def make_paths():
			...
			for fid in range(3):
				...
				yield Blob(rw_repo, Blob.NULL_BIN_SHA, 100644, fname)
